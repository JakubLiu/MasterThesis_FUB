MCTPWB <- function(data,mu0,n_iter){
  ...
  t_stats <- (col_means - mu0) / (col_sds / sqrt(n))  # calculate the column t-statistics (d*n_iter)
  t_matrix <- matrix(t_stats, nrow = d, ncol = n_iter)  # reshape the t-statistics vector into a mtrix of shape (d,n_iter)
