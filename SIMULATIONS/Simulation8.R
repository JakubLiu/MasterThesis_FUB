...
variances <- seq(from = 0.0, to = 10.0, by = 1.0)
...
n_conditions <- length(covariance_structures) * length(variances) * length(sample_sizes)
n_iter_maxtest <- 10
n_simul <- 10
...
param_grid <- expand.grid(sample_sizes, variances, covariance_structures)
colnames(param_grid) <- c('sample_sizes', 'variances', 'covariance_structures')
param_grid <- param_grid[,c('covariance_structures','variances','sample_sizes')]

# create a placeholder for the output simulation table
simulation_table <- data.frame(matrix(NA, nrow = 1, ncol = 5))
colnames(simulation_table) <- c('covariance_structures', 'variances', 'sample_sizes', 'power_global', 'power_max')
...
for(condition in 1:nrow(param_grid)){
  ...
  variance_ <- param_grid$variances[condition]
