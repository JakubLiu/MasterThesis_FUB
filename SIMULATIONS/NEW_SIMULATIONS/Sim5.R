...
means_concave <- f(means_linear, -20, 41, -10) # the parameters for the quadratic function where chosen by inspecting the scatterplots of the convex means
...
for(i in 1:length(var_sample_sizes)){
  ...
  pvalues <- foreach(i=1:const_nsim, .packages = c("MASS", "resample"), .combine = rbind) %dopar%{
    ...
    X <- GenDataGivenMeans(means_concave,n)
...
plot(sim_tab$var_sample_sizes, sim_tab$powers_glob, col = 'blue', type = 'l', lwd = 4, ylim = c(0,1))
lines(sim_tab$var_sample_sizes, sim_tab$powers_MCTP, col = 'red', type = 'l', lwd = 4)
grid()
